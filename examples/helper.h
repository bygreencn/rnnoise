...
drwav* init_wavfile(const char* filename, uint16_t channels, uint16_t sampleRate, uint16_t bitsPerSample, uint16_t format){
    drwav_data_format drformat;
    drformat.container = drwav_container_riff;     // <-- drwav_container_riff = normal WAV files, drwav_container_w64 = Sony Wave64.
    drformat.format = format;          // <-- Any of the DR_WAVE_FORMAT_* codes.
    drformat.channels = channels;
    drformat.sampleRate = sampleRate;
    drformat.bitsPerSample = bitsPerSample;
    drwav* pWav = drwav_open_file_write(filename, &drformat);
    if (pWav == NULL) {
...
uint64_t write_frames_f32(drwav* pWav, uint64_t frameCount, float* pSamples){
    ...
    for (size_t i = 0; i < frameCount; ++i) {
        framesWritten += drwav_write_pcm_frames(pWav, 1, pSamples+i);
